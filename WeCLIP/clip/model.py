...
def build_model(state_dict: dict,
    ...
    if use_lora:
        inject_lora_to_transformer(model.transformer, r=lora_r, alpha=lora_alpha, enable_lora=enable_lora)  # text
        if isinstance(model.visual, VisionTransformer):
            inject_lora_to_transformer(model.visual.transformer, r=lora_r, alpha=lora_alpha, enable_lora=enable_lora)  # image
...
def inject_lora_to_transformer(transformer_module, r=4, alpha=2, enable_lora=['q','k','v']):
    for block in transformer_module.resblocks:
        block.attn = PlainMultiheadAttentionLoRA(
            existing_mha=block.attn,
